...
const SplashCursor: React.FC<SplashCursorProps> = ({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement): WebGLContextResult | null {
      ...
      const isWebGL2 = !!gl;
      let webgl1: WebGLRenderingContext | null = null;
      if (!isWebGL2) {
        webgl1 = (canvas.getContext('webgl', params) || canvas.getContext('experimental-webgl', params)) as WebGLRenderingContext | null;
      }
      const activeGl = (gl || webgl1);
      ...
      if (isWebGL2 && gl) {
        ...
        halfFloat = webgl1.getExtension('OES_texture_half_float');
        supportLinearFiltering = webgl1.getExtension('OES_texture_half_float_linear');
      }
      activeGl.clearColor(0.0, 0.0, 0.0, 1.0);

      const halfFloatTexType = isWebGL2
        ? (activeGl as WebGL2RenderingContext).HALF_FLOAT
      ...
      if (isWebGL2 && gl) {
        ...
        formatRGBA = getSupportedFormat(activeGl, activeGl.RGBA, activeGl.RGBA, halfFloatTexType);
        formatRG = getSupportedFormat(activeGl, activeGl.RGBA, activeGl.RGBA, halfFloatTexType);
        formatR = getSupportedFormat(activeGl, activeGl.RGBA, activeGl.RGBA, halfFloatTexType);
      }
    ...
    function getUniforms(program: WebGLProgram): { [name: string]: WebGLUniformLocation | null } {
    ...
    // Fixed HSVtoRGB parameter mismatch
